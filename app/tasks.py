...
@app.task
def ejecutar_script(script: str):
    """Ejecuta un script de shell y devuelve su salida."""
    execution_list = ['bash'] + script.split(" ")
    result = subprocess.run(execution_list, capture_output=True, text=True, check=False)
    return result.stdout


@app.task
def ejecutar_script_python(script: str):
    """Ejecuta un script de Python y devuelve su salida."""
    execution_list = ['python3'] + script.split(" ")
    result = subprocess.run(execution_list, capture_output=True, text=True, check=False)
    return result.stdout
